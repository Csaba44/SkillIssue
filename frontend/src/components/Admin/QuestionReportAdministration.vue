...
const filteredReports = computed(() => {
  ...
  return props.reports
    .filter((r) => {
      ...
      return !search || r.question?.question.toLowerCase().includes(search) || r.user?.name.toLowerCase().includes(search);
    })
  ...
  <div class="flex flex-col gap-8">
    <div class="flex flex-col lg:flex-row justify-between items-center gap-6">
      <div class="relative w-full lg:w-96">
        ...
        <input v-model="searchQuery" type="text" placeholder="Szűrés (kérdés, felhasználó...)" class="w-full bg-white/5 border border-white/10 rounded-2xl py-3 pl-12 pr-4 text-white outline-none focus:border-red-500/50 transition-all" />
      </div>
    ...
    <div class="grid gap-4">
      <div v-if="filteredReports.length > 0" class="grid gap-3">
        <QuestionReportItem v-for="report in filteredReports" :key="report.id" :report="report" :class="{ 'opacity-50 grayscale': report.status == 'Closed' }" @delete="emit('deleteReport', report.id)" @view="openDetails" />
      </div>
