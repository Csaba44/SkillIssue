...
const filteredReports = computed(() => {
  ...
  return props.reports
    .filter((r) => {
      ...
      return (
        !search || r.question?.question.toLowerCase().includes(search) || r.user?.name.toLowerCase().includes(search)
      );
    })
    ...
    <div class="flex flex-col lg:flex-row justify-between items-center gap-6">
      <div class="relative w-full lg:w-96">
        ...
        <input
          v-model="searchQuery"
          type="text"
          placeholder="Szűrés (kérdés, felhasználó...)"
    ...
    <div class="grid gap-4">
      <div v-if="filteredReports.length > 0" class="grid gap-3">
        <QuestionReportItem
          ...
          @delete="emit('deleteReport', report.id)"
          @view="openDetails"
        />
      </div>
        ...
        <div class="space-y-6 text-left">
          <div>
            ...
            <div class="flex flex-col gap-2">
              <div
                v-for="answer in selectedReport.question?.answers"
                :key="answer.id"
                class="flex items-center gap-3 px-4 py-3 rounded-xl border text-sm transition-all"
                :class="
              ...
              >
                <i
                  ...
                  :class="answer.answer.endsWith('*') ? 'fa-circle-check text-accentGreen' : 'fa-circle text-white/20'"
                ></i>
                <span>{{ answer.answer.endsWith("*") ? answer.answer.slice(0, -1) : answer.answer }}</span>
                <span
                  v-if="answer.answer.endsWith('*')"
                  class="ml-auto text-[10px] uppercase font-bold tracking-widest text-accentGreen/70"
                  >Helyes</span
                >
              </div>
            </div>
          ...
          <div>
            <label class="text-white/30 text-[10px] uppercase font-bold tracking-widest block mb-2"
              >Felhasználó indoklása</label
            >
            <div class="text-red-200/60 bg-red-500/5 p-4 rounded-xl border border-red-500/10 italic text-sm">
          ...
          <div>
            <label class="text-white/20 text-[10px] uppercase font-bold tracking-widest block mb-2"
              >Admin megjegyzés (Saját jegyzet)</label
            >
            <textarea
              v-model="adminNote"
              placeholder="Ide írd a belső jegyzeteidet..."
              rows="3"
          ...
          <div>
            ...
            <div class="grid grid-cols-3 gap-2">
              ...
              <button
                ...
                :class="
                  activeStatus == 'Closed'
                    ? 'bg-accentGreen text-black shadow-lg shadow-accentGreen/20'
                    : 'bg-white/5 text-white/40 hover:bg-white/10'
                "
                class="py-3 rounded-xl font-bold uppercase text-[10px] transition-all"
              >
        ...
        <div class="flex justify-between items-center mt-10 pt-6 border-t border-white/5 sm:flex-row flex-col">
          ...
          <div class="flex gap-4 sm:mt-0 mt-2">
            ...
            <button
              ...
              class="bg-accentGreen text-black font-extrabold px-8 py-2 rounded-full text-xs hover:scale-105 active:scale-95 transition-all shadow-lg shadow-accentGreen/20 disabled:opacity-50 disabled:cursor-not-allowed"
            >
