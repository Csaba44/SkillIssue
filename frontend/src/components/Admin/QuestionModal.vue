...
const formData = ref({
  ...
  answers:
    props.editData && props.editData.answers && props.editData.answers.length > 0
      ...
      <div class="space-y-6 text-left">
        <div v-if="!isEdit">
          <div class="flex justify-between items-center mb-2">
            ...
            <button @click="isAddingNewSubject = !isAddingNewSubject" type="button" class="text-[10px] uppercase font-bold text-accentGreen hover:underline cursor-pointer">
              {{ isAddingNewSubject ? "Inkább választok a listából" : "+ Új tantárgy létrehozása" }}
            </button>
          </div>

          <Select v-if="!isAddingNewSubject" :options="subjectsArray" class="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-white outline-none focus:border-accentGreen transition-all" v-model="formData.subject_id"> </Select>

          <input v-else v-model="newSubjectName" type="text" placeholder="Új tantárgy neve (pl. Matek)" class="w-full bg-white/5 border border-accentGreen/50 rounded-xl p-4 text-white outline-none focus:border-accentGreen transition-all shadow-[0_0_15px_rgba(34,197,94,0.1)]" />
        </div>

        <div>
          <label class="text-white/40 text-[10px] uppercase font-bold tracking-widest block mb-2">Kérdés</label>
          <textarea v-model="formData.question" placeholder="Írd be a kérdést..." rows="2" class="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-white outline-none focus:border-accentGreen transition-all resize-none" />
        </div>

        <div class="space-y-3">
          <label class="text-white/40 text-[10px] uppercase font-bold tracking-widest block">Válaszok (Az első a helyes)</label>
          <div v-for="(ans, i) in formData.answers" :key="i" class="flex items-center gap-3">
            <div :class="i == 0 ? 'bg-accentGreen shadow-[0_0_10px_rgba(34,197,94,0.3)]' : 'bg-white/10'" class="w-1.5 h-10 rounded-full"></div>
            <input v-model="ans.answer" class="flex-1 bg-white/5 border border-white/10 rounded-xl p-3 text-white outline-none text-sm" />
          </div>
        </div>
