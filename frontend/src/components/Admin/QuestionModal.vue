      ...
      <div class="space-y-6 text-left">
        <div v-if="!isEdit">
          <div class="flex justify-between items-center mb-2">
            ...
            <button
              @click="isAddingNewSubject = !isAddingNewSubject"
              type="button"
        ...
        <div class="space-y-3">
          <label class="text-white/40 text-[10px] uppercase font-bold tracking-widest block"
            >Válaszok (Az első a helyes)</label
          >
          <div v-for="(ans, i) in formData.answers" :key="i" class="flex items-center gap-3">
            <div
              :class="i == 0 ? 'bg-accentGreen shadow-[0_0_10px_rgba(34,197,94,0.3)]' : 'bg-white/10'"
        ...
        <div class="flex justify-end gap-4 pt-6 sm:flex-row flex-col">
          <button
            @click="emit('close')"
            class="px-6 py-3 text-white/40 font-bold uppercase text-xs transition-all duration-200 ease-in-out hover:text-white hover:scale-105 active:scale-95 active:opacity-70 cursor-pointer"
          >
            Mégse
          </button>
          <Button
