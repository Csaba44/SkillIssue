  ...
  <div class="flex flex-col gap-8">
    <div class="flex flex-col lg:flex-row justify-between items-center gap-6">
      <div class="relative w-full lg:w-96">
        ...
        <input v-model="searchQuery" type="text" placeholder="Felhasználó vagy indok keresése..." class="w-full bg-white/5 border border-white/10 rounded-2xl py-3 pl-12 pr-4 text-white outline-none focus:border-red-500/50 transition-all" />
      </div>
    ...
    <div class="grid gap-3">
      <div v-if="isLoading" class="py-20 text-center text-white/20 font-medium"><i class="fa-solid fa-spinner animate-spin mr-2"></i> Betöltés...</div>
    ...
    <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
      <div class="bg-bgDark border border-white/10 w-full max-w-xl rounded-3xl sm:p-8 p-3 shadow-2xl overflow-y-auto max-h-[95vh]">
        <div class="flex justify-between items-start mb-8">
          <div>
            <h2 class="text-xl font-bold text-white flex items-center gap-3"><i class="fa-solid fa-ban text-red-500"></i> Ban részletei</h2>
            <div class="mt-2">
              <span class="text-[10px] uppercase font-black px-2 py-0.5 rounded tracking-widest" :class="isExpired(selectedBan.release_date) ? 'bg-white/5 text-white/30' : 'bg-red-500/10 text-red-400'">
        ...
        <div class="space-y-6 text-left">
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="text-white/30 text-[10px] uppercase font-bold tracking-widest block mb-1">Kitiltott felhasználó</label>
              <p class="text-white font-medium">{{ selectedBan.user?.name ?? "Ismeretlen" }}</p>
            </div>
            <div>
              <label class="text-white/30 text-[10px] uppercase font-bold tracking-widest block mb-1">Ban létrehozva</label>
          ...
          <div>
            ...
            <input v-model="editForm.reason" type="text" placeholder="Ban oka..." class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm outline-none focus:border-red-500/40 transition-all" />
          </div>

          <div>
            <label class="text-white/30 text-[10px] uppercase font-bold tracking-widest block mb-2">Lejárat dátuma</label>
            <input v-model="editForm.release_date" type="datetime-local" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm outline-none focus:border-red-500/40 transition-all" />
          </div>
        </div>

        <div class="flex justify-between items-center mt-10 pt-6 border-t border-white/5 sm:flex-row flex-col">
          <button @click="deleteBan(selectedBan.id)" class="group flex items-center gap-2 px-4 py-2 text-red-500/70 hover:text-red-500 hover:bg-red-500/5 rounded-xl cursor-pointer transition-all duration-300 ease-out active:scale-95 text-[10px] font-bold uppercase">
          ...
          <div class="flex gap-4 sm:mt-0 mt-2">
            <button @click="isModalOpen = false" class="px-6 py-2 text-white/40 font-bold uppercase text-xs cursor-pointer hover:text-white hover:-translate-y-0.5 transition-all duration-200 ease-in-out">Bezárás</button>
            <button @click="saveUpdate()" :disabled="isSubmitting" class="bg-accentGreen text-black font-extrabold px-8 py-2 rounded-full text-xs hover:scale-105 transition-all shadow-lg shadow-accentGreen/20 disabled:opacity-50 disabled:cursor-not-allowed disabled:scale-100">
