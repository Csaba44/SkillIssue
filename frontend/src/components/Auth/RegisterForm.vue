...
const registerSubmit = async () => {
  ...
  if (!formData.value.accepted) return toastErr("El kell fogadnia az adatkezelési tájékoztatót.");
    ...
    formData.value.password2.trim() == ""
  )
    return toastErr("Minden mező kitöltése kötelező.");
...
<template>
  <form
    ...
    class="row-span-5 lg:row-span-4 py-5 flex flex-col gap-3 items-center justify-center h-full w-full"
  >
    <Input
    ...
    <div class="lg:max-w-105">
      <Checkbox
        ...
        ><span class="text-textWhite text-xs">
          Kijelentem, hogy az
          <router-link to="/privacy-policy" target="_blank" class="text-accentYellow cursor-pointer hover:underline">
