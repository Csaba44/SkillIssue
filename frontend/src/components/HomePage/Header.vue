...
<template>
  ...
  >
    ...
    >
      <i
        class="fa-solid fa-graduation-cap text-[700px] text-accentPurple opacity-5 absolute top-30 left-15 rotate-20"
      ></i>
      <h1 class="mt-10 md:mt-70 text-4xl lg:mt-0 xl:mt-0 md:text-6xl xl:text-8xl font-bold text-white text-nowrap">
        Érettségire fel!
      </h1>
      <h2 class="text-2xl sm:mt-5 md:text-xl lg:text-3xl mt-3 font-bold text-white w-full">
        Játssz, tanulj és készülj az érettségire!
      </h2>
      <RouterLink :to="isAuthenticated ? '/dashboard' : '/login?register=1'"
        ...
        <div class="flex">
          <div class="flex">
            <TopicCard
              ...
              v-show="!card.isInvisibleOnHeader"
              :key="index"
              :icon="card.icon"
