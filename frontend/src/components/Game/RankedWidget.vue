...
const createQuestionReportClicked = () => {
  ...
  setTimeout(() => {
    window.open(
      `/report/question?questiontoken=${currentQuestionToken.value}&question=${currentQuestion.value}&answers=${encodeURIComponent(JSON.stringify(currentAnswers.value))}`,
      "_blank",
    );
  }, 1000);
};
...
const createUserReportClicked = () => {
  ...
  setTimeout(() => {
    window.open(
      `/report/user?rankedtoken=${gameStore.match.ranked_token}&opponent=${opponent.value.player.userName ?? "Ismeretlen ellenfél"}&reportround=${currentRound.value ?? TOTAL_ROUNDS}`,
      "_blank",
    );
  }, 1000);
};
...
<template>
  ...
  >
    ...
    <div class="w-full flex flex-col gap-4 mt-4" :class="isSubmitting ? 'opacity-60 pointer-events-none' : ''">
      <template v-for="answer in currentAnswers" :key="answer.id">
        <AnswerButton
          ...
          :isOpponentAnswer="opponentAnswerId !== null && answer.id === opponentAnswerId"
        >
    ...
    <div
      v-if="showWaiting && correctAnswerId === null"
      class="flex flex-col items-center gap-3 text-white/80 text-center"
    >
      <div class="flex items-center gap-3">
