...
<template>
  <ProtectedPageContainer class="grid grid-rows-3 grid-cols-1 lg:grid-rows-1 lg:grid-cols-2 p-0!">
    <div class="items-center justify-center border-b lg:border-b-0 lg:border-r border-white/25 relative">
      <div
        class="absolute inset-0 h-full w-full opacity-20 bg-[radial-gradient(#ffffff_1px,transparent_1px)] bg-size-[16px_16px] z-0"
      ></div>
      <div class="z-10 grid grid-cols-1 grid-rows-5 p-10 h-full">
        <div
          class="row-span-5 lg:row-span-2 z-10 text-2xl lg:text-5xl font-bold flex flex-col text-center gap-5 justify-center items-center"
        >
        ...
        <div class="hidden lg:visible row-span-3 lg:grid grid-rows-1 grid-cols-1 z-10 items-center text-center">
          <div>
            ...
            <div class="flex items-center justify-center">
              <TopicCard
                v-for="(card, index) in literatureCards"
                :key="index"
                :icon="card.icon"
                :iconTop="card.iconTop"
                :iconLeft="card.iconLeft"
            ...
            <div class="flex items-center justify-center">
              <TopicCard
                v-for="(card, index) in grammarCards"
                :key="index"
                :icon="card.icon"
                :iconTop="card.iconTop"
                :iconLeft="card.iconLeft"
    ...
    <div class="row-span-2 lg:row-span-1 grid grid-cols-1 grid-rows-6 p-10 h-full items-center justify-center">
      <div
        class="row-span-1 lg:row-span-2 text-2xl lg:text-5xl font-bold text-textWhite flex flex-col text-center gap-5 justify-center items-center"
      >
        {{ currFormToShow == "login" ? "Bejelentkezés" : "Regisztráció" }}
      </div>

      <LoginForm
        v-if="currFormToShow == 'login' && !userStore.isAuthenticated"
        @switch-form="currFormToShow = 'register'"
      ></LoginForm>
      <RegisterForm
        v-if="currFormToShow == 'register' && !userStore.isAuthenticated"
        @switch-form="currFormToShow = 'login'"
      ></RegisterForm>
    </div>
