  ...
  <ProtectedPageContainer class="relative overflow-hidden">
    ...
    <section class="mt-20 flex flex-col items-center text-center text-textWhite">
      ...
      <div class="flex flex-col sm:flex-row gap-8">
        <div
          @click="setSelected('Ranked')"
          :class="
            selectedGameMode === 'Ranked' ? 'border-primary bg-primary/10 scale-105' : 'border-white/10 bg-white/5'
          "
          class="cursor-pointer w-60 p-6 rounded-2xl border backdrop-blur-lg transition-all duration-300 hover:scale-105"
        >
      ...
      <div class="mt-12 flex flex-col items-center gap-6">
        <button
          ...
          class="px-14 py-5 text-xl font-bold rounded-full bg-linear-to-r from-accentGreen to-success text-black shadow-lg shadow-green-500/30 hover:scale-105 transition-all duration-300 disabled:opacity-40 disabled:cursor-not-allowed"
        >
        ...
        <button
          v-else
          @click="stopMatchmaking"
          :disabled="selectedGameMode === 'Solo'"
          class="px-14 py-5 text-xl font-bold rounded-full bg-linear-to-r from-red-500 to-error text-white shadow-lg shadow-red-500/30 hover:scale-105 transition-all duration-300 disabled:opacity-40 disabled:cursor-not-allowed"
        >
      ...
      <div class="mt-3 text-accentGreen text-sm bg-accentGreen/10 px-4 py-2 rounded-full">
        ● {{ mm.playersInQueue }} játékos meccskeresésben
      </div>
    </section>

    <section class="mt-10 grid sm:grid-cols-2 md:grid-cols-4 gap-8 max-w-6xl mx-auto">
      <div
        class="p-6 rounded-2xl bg-white/5 backdrop-blur-xl border border-yellow-400/20 hover:shadow-yellow-400/20 hover:shadow-lg transition-all"
      >
      ...
      <div
        class="p-6 rounded-2xl bg-white/5 backdrop-blur-xl border border-purple-400/20 hover:shadow-purple-400/20 hover:shadow-lg transition-all"
      >
      ...
      <div
        class="p-6 rounded-2xl bg-white/5 backdrop-blur-xl border border-green-400/20 hover:shadow-green-400/20 hover:shadow-lg transition-all"
      >
      ...
      <div
        class="p-6 rounded-2xl bg-white/5 backdrop-blur-xl border border-orange-400/20 hover:shadow-orange-400/20 hover:shadow-lg transition-all"
      >
